...
async function main() {
    ...
    while (true) {
        ...
        // === LOGIN ===
        if (opcion === "acceder") {
            const nombre = await (0, prompts_1.input)({ message: "Ingrese su nombre:" });
            const correo = await (0, prompts_1.input)({ message: "Ingrese su correo:" });
            ...
            if (usuarioActual.tipo === "profesor") {
                while (true) {
                    const opcionProfesor = await (0, prompts_1.select)({
                    ...
                    if (opcionProfesor === "crear_curso") {
                        const titulo = await (0, prompts_1.input)({ message: "Título del curso:" });
                        ...
                        const tablaCursos = plataforma.obtenerCursosProfesor(usuarioActual.correo);
                        console.table(tablaCursos);
                    }
                    else if (opcionProfesor === "ver_alumnos") {
                        const cursosConEstudiantes = plataforma.obtenerEstudiantesDeCursos(usuarioActual.correo);
            ...
            if (usuarioActual?.tipo === "estudiante") {
                while (true) {
                    const opcionEstudiante = await (0, prompts_1.select)({
                    ...
                    if (opcionEstudiante === "mis_cursos") {
                        const cursos = plataforma.obtenerCursosEstudiante(usuarioActual.correo);
                        ...
                        console.log("📖 Cursos disponibles:", cursos.map((c) => c.titulo));
                        const cursoTitulo = await (0, prompts_1.input)({
        ...
        // === REGISTRO ===
            ...
            const nombre = await (0, prompts_1.input)({ message: "Ingrese su nombre:" });
            const correo = await (0, prompts_1.input)({ message: "Ingrese su correo:" });
            const tipo = await (0, prompts_1.select)({
